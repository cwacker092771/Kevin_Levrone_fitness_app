...
function importMetricsCsv(csvText) {
  ...
  // No date/time column in the file — every row gets today's date instead.
  matchedColumns.date = dateIdx !== -1 ? rawHeaders[dateIdx] : "(none found — using today's date)";

  const hasAnyMetricColumn = Object.keys(columnIdxByKey).length > 0;
  if (!hasAnyMetricColumn) {
    return {
      matchedColumns,
      entries: [],
      skipped: [],
      error: "Could not find at least one recognizable metric column. Found headers: " + rawHeaders.join(", ")
    };
  }
