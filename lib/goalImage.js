...
// img2img strength. Kept low so the person's face carries through - the build
// shift is modest by design. Raising it makes a bigger transformation but the
// face drifts into someone else.
...
function buildPrompt(goal) {
  ...
  return (
    `The exact same person, same face, same hair, same skin tone - unmistakably ` +
    `them - a few months from now after training, having reached ${physique}. ` +
    `Same head and pose, athletic fitted clothing, warm flattering light, ` +
    `photorealistic portrait, sharp focus, natural.`
  );
}
...
async function generateGoalImage(goal, avatarBuffer) {
  ...
  const body = {
    ...
    negative_prompt: "different person, different face, face swap, stranger, unrecognizable, distorted face, deformed anatomy, extra limbs, text, watermark, blurry",
    mode: "image-to-image",
